...
class Spectra(ABC):
    ...
    def construct_spectra_graph(self, save_path, force_reconstruct=False):
        if self.SPG is not None:
            return self.SPG
                ...
                f"{save_path}_spectral_property_graphs/{self.dataset.name}_SPECTRA_property_graph.gexf") and not force_reconstruct:
            print("Loading spectral property graph")
            self.SPG = nx.read_gexf(
                f"{save_path}_spectral_property_graphs/{self.dataset.name}_SPECTRA_property_graph.gexf")
            ...
            if self.spectra_properties_loaded is not None:
                ...
                for i in tqdm(range(len(self.dataset))):
                    for j in range(i + 1, len(self.dataset)):
                        if self.spectra_properties_loaded is not None:
                            weight = self.spectra_properties_loaded[
                                (self.spectra_properties_loaded[0] == i) & (self.spectra_properties_loaded[1] == j)][
                                2].values[0]
                        else:
            ...
            if self.binary:
                ...
                if is_clique(self.SPG):
                    ...
                    raise Exception(
                        "The SPG is fully connected, cannot run SPECTRA, all samples are similar to each other")
                else:
                    ...
                    if all_fully_connected:
                        raise Exception(
                            "All SPG sub components are fully connected, cannot run SPECTRA, all samples are similar to each other")

            if not os.path.exists(f"{save_path}_spectral_property_graphs"):
                os.makedirs(f"{save_path}_spectral_property_graphs")

            nx.write_gexf(self.SPG,
                          f"{save_path}_spectral_property_graphs/{self.dataset.name}_SPECTRA_property_graph.gexf")
    ...
    def generate_spectra_split(self,
        ...
        stats = {'SPECTRA_parameter': spectral_parameter, 'train_size': len(train), 'test_size': len(test),
    ...
    def generate_spectra_splits(self,
        ...
        if self.binary:
            if nx.density(self.SPG) >= 0.4:
                raise Exception(
                    "Density of SPG is greater than 0.4, SPECTRA will not work as your dataset is too similar to itself. Please check your dataset and SPECTRA properties.")
        ...
        for spectral_parameter in spectral_parameters:
            for i in range(number_repeats):
                if os.path.exists(f"{save_path}_SPECTRA_splits/SP_{spectral_parameter}_{i}") and not force_reconstruct:
                    ...
                    train, test, stats = self.generate_spectra_split(float(spectral_parameter), random_seed[i],
                                                                     test_size)
                    if train is not None:
                        ...
                        pickle.dump(train,
                                    open(f"{save_path}_SPECTRA_splits/SP_{spectral_parameter}_{i}/train.pkl", "wb"))
                        pickle.dump(test,
                                    open(f"{save_path}_SPECTRA_splits/SP_{spectral_parameter}_{i}/test.pkl", "wb"))
                        pickle.dump(stats,
                                    open(f"{save_path}_SPECTRA_splits/SP_{spectral_parameter}_{i}/stats.pkl", "wb"))
                    else:
                        print(
                            f"Split for SP_{spectral_parameter}_{i} could not be generated since independent set only has one sample")
    ...
    def return_split_samples(self, spectral_parameter, number, data_path=""):
        split_folder = f"{data_path}_SPECTRA_splits/SP_{spectral_parameter}_{number}"
        if not os.path.exists(split_folder):
            raise Exception(f"Split folder {split_folder} does not exist")
        else:
            train = pickle.load(open(f"{split_folder}/train.pkl", "rb"))
            test = pickle.load(open(f"{split_folder}/test.pkl", "rb"))
            return [int(i) for i in train], [int(i) for i in test]
    ...
    def return_all_split_stats(self):
        ...
        stats = {'SPECTRA_parameter': SP, 'number': number, 'train_size': train_size, 'test_size': test_size,
                 'cross_split_overlap': cross_split_overlap}
        return stats
    ...
    def pre_calculate_spectra_properties(self, save_path, force_recalculate=False):
        if os.path.exists(f"{save_path}_precalculated_spectra_properties") and not force_recalculate:
            ...
            similarity_file = open(f"{save_path}_precalculated_spectra_properties", 'w')
        ...
        self.load_spectra_precalculated_spectra_properties(save_path)
    ...
    def load_spectra_precalculated_spectra_properties(self, filename):
        if not os.path.exists(f"{filename}_precalculated_spectra_properties"):
            ...
            self.spectra_properties_loaded = pd.read_csv(f"{filename}_precalculated_spectra_properties", sep='\t',
                                                         header=None)
        ...
        def lookup_spectra_property(x, y):
            if not is_integer(x) or not is_integer(y):
                ...
                res1 = self.spectra_properties_loaded[
                    (self.spectra_properties_loaded[0] == x) & (self.spectra_properties_loaded[1] == y)]
                res2 = self.spectra_properties_loaded[
                    (self.spectra_properties_loaded[0] == y) & (self.spectra_properties_loaded[1] == x)]
                if len(res1) > 0:
