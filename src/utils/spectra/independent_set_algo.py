...
def run_independent_set(spectral_parameter, input_G, seed=None,
    ...
    if binary:
        ...
        for i, component in enumerate(components):
            ...
            if is_clique(subgraph):
                print(
                    f"Component {i} is too densly connected, adding samples as a single unit to independent set and deleting them from the graph")
    ...
    while not nx.is_empty(G):
            ...
            if not binary:
                if spectral_parameter == 1.0:
                    ...
                    if distribution is None:
                        raise Exception(
                            "Distribution must be provided if binary is set to False, must precompute similarities")
                    if random.random() < spectral_parameter and (
                            1 - spectral_parameter) * 100 < stats.percentileofscore(distribution, edge_weight):
                        neighbors_to_delete.append(neighbor)
            else:
                if spectral_parameter == 1.0:
                    neighbors_to_delete.append(neighbor)
        ...
        if debug:
            ...
            print(
                f"Deleted {len(neighbors_to_delete)} nodes from {G.degree(chosen_node)} neighbors of node {chosen_node}")
